...
1. **Project configuration:** Each project defines its concerns in `cross-cutting-concerns.yaml` at the project root (or in the hive state directory). This file is loaded during `/hive:plan` and `/hive:execute`.
...
Place `cross-cutting-concerns.yaml` at the project root or in `state/`. Hive loads it during plan and execute commands.
